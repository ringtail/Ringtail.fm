...
ContentFlow.prototype = {
    ...
    _defaultConf: { 
        ...
        onclickActiveItem: function (item) {
            ...
            //     $('#channel_menu').slideUp(function(){
            //         $('#player').slideDown();

            //         MusicContent.getList(status.theme,,);
            //         Box.create(status.theme);
            //         $('.wheel').addClass('wheel_left_active');
            //         $('#cassette_appearance').addClass('cassette_appearance_active');
            //     })
            var selected_index = item.index;
            //this is for test
            console.log('你选择了'+selected_index);
            $('#channel_menu').slideUp(function(){
                MusicContent.getList(status.theme,data[selected_index].origin,data[selected_index].channel);
                Box.create(status.theme);
                $('#player').slideDown(1000);
                $('div.wheel').addClass('wheel_left_active');
